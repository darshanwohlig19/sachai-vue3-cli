...
import Carousel from "primevue/carousel"; // Import Carousel from PrimeVue
  ...
  methods: {
    async fetchProducts() {
      try {
        ...
        this.desktopProducts = desktopData.slice(0, 9) || []; // Fallback to empty array if no data

        // Fetching products for mobile
        const mobileData = await MobileService.getProductsSmall();
        this.mobileProducts = mobileData.slice(0, 9) || []; // Fallback to empty array if no data
      } catch (error) {
    ...
    togglePhoneVerification() {
      this.showPhoneVerification = !this.showPhoneVerification;
      this.verificationCodeTab = false;
    },
    handleSendVerificationCode() {
      // Your logic to send verification code here
      this.verificationCodeTab = true;
    },
    verifyCode() {
